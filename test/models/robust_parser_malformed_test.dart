...
void main() {
  group('D3 HolidayEntry/HolidayData robust parsing', () {
    test('HolidayEntry.fromJson rejects missing date', () {
      expect(() => HolidayEntry.fromJson({'name': 'test', 'type': 'vacation'}), throwsA(isA<FormatException>()));
    });
    test('HolidayEntry.fromJson rejects bad date string', () {
      expect(() => HolidayEntry.fromJson({'date': 'not-a-date', 'name': 'x', 'type': 'vacation'}), throwsA(isA<FormatException>()));
    });
    test('HolidayEntry.fromJson rejects missing name', () {
      expect(() => HolidayEntry.fromJson({'date': '2026-10-01', 'type': 'vacation'}), throwsA(isA<FormatException>()));
    });
    test('HolidayData.fromJson skips malformed entry, keeps valid', () {
      final data = HolidayData.fromJson({
        ...
        'version': 1,
        'entries': [
          {'date': '2026-10-01', 'name': '国庆', 'type': 'vacation', 'groupId': 'holiday-2026-0'},
          {'date': 'bad-date', 'name': 'bad', 'type': 'vacation'},
          {'name': 'missing-date', 'type': 'vacation'},
          {'date': '2026-10-02', 'name': '国庆', 'type': 'vacation', 'groupId': 'holiday-2026-0'},
          'not-a-map',
        ],
      });
    ...
    test('HolidayData preserves year string coercion and skips invalid year', () {
      expect(HolidayData.fromJson({'year': '2026', 'entries': []}).year, 2026);
      expect(() => HolidayData.fromJson({'year': 'bad', 'entries': []}), throwsA(isA<FormatException>()));
    });
    test('HolidayData grouping/precedence: custom overrides makeup', () {
      final data = HolidayData.fromJson({
        ...
        'entries': [
          {'date': '2026-10-01', 'name': '国庆', 'type': 'vacation', 'groupId': 'holiday-2026-0'},
          {'date': '2026-10-01', 'name': '学校补班', 'type': 'adjusted_workday', 'groupId': 'custom-makeup-1'},
  ...
  group('B2 holiday_service remote converters skip malformed', () {
    test('convertApiEntries skips bad date/daytype and keeps valid', () {
      ...
      expect(entries.where((e) => e.type == HolidayType.adjustedWorkday).length, 1);
      svc.dispose();
    });
  });
  ...
  group('D5 TimeScheme robust parsing', () {
    test('skips malformed SectionTime, keeps valid; preserves required id', () {
      ...
      expect(scheme.sections.map((s) => s.displayText), ['08:00-08:45', '10:00-10:45']);
    });
    test('TimeScheme rejects missing id (no silent invention)', () {
      expect(() => TimeScheme.fromJson({'name': 'x', 'sections': []}), throwsA(isA<FormatException>()));
      expect(() => TimeScheme.fromJson({'id': '', 'sections': []}), throwsA(isA<FormatException>()));
    });
  });

  group('D7 SectionTime/SavedTheme/TimetableSettings robust parsing', () {
    test('SectionTime rejects missing fields', () {
      expect(() => SectionTime.fromJson({'startTime': '08:00'}), throwsA(isA<FormatException>()));
      expect(() => SectionTime.fromJson({'endTime': '08:45'}), throwsA(isA<FormatException>()));
    });
    test('SavedTheme rejects missing id/name/createdAt', () {
      expect(() => SavedTheme.fromJson({'name': 'n', 'themeData': {}, 'createdAt': '2026-01-01T00:00:00.000'}), throwsA(isA<FormatException>()));
      expect(() => SavedTheme.fromJson({'id': 'a', 'themeData': {}, 'createdAt': '2026-01-01T00:00:00.000'}), throwsA(isA<FormatException>()));
      expect(() => SavedTheme.fromJson({'id': 'a', 'name': 'n', 'themeData': {}, 'createdAt': 'bad'}), throwsA(isA<FormatException>()));
    });
    test('TimetableSettings skips malformed sections/themes, preserves semester/theme', () {
      final s = TimetableSettings.fromJson({
        ...
        'savedThemes': [
          {'id': 't1', 'name': 'good', 'themeData': {'v': 2, 'seed': '#00FF00'}, 'createdAt': '2026-01-01T00:00:00.000'},
    ...
    test('TimetableSettings falls back to defaults when all sections malformed', () {
      final s = TimetableSettings.fromJson({
        'sections': [
          {'startTime': '', 'endTime': ''},
        ],
  ...
  group('D8 WarehouseMacro robust parsing', () {
    test('WarehouseMacroRecord skips malformed steps, keeps valid', () {
      ...
      expect(rec.steps.map((e) => e.type), containsAll([MacroStepType.navigate, MacroStepType.click]));
    });
    test('WarehouseMacroRecord dialogResponses tolerates non-map', () {
      final rec = WarehouseMacroRecord.fromJson({
        'schoolId': '', 'adapterId': '', 'schoolName': '', 'adapterName': '', 'importUrl': '', 'schoolResourceFolder': '', 'adapterAssetJsPath': '',
        'steps': [],
