  ...
  group('effectiveCourseCardSurfaceStyle', () {
    ...
    test(
      ...
      () async {
        ...
        final dir = await Directory.systemTemp.createTemp('effective_style_blur');
        final file = File('${dir.path}/wall.png')..writeAsBytesSync([1, 2, 3, 4]);
        addTearDown(() => dir.deleteSync(recursive: true));
        final settings = TimetableSettings.defaults().copyWith(
          homePageWallpaperPath: file.path,
    ...
    test('blur pipeline unavailable: solid stays solid', () async {
      final dir = await Directory.systemTemp.createTemp('effective_style_blur2');
      final file = File('${dir.path}/wall.png')..writeAsBytesSync([1, 2, 3, 4]);
      addTearDown(() => dir.deleteSync(recursive: true));
      final settings = TimetableSettings.defaults().copyWith(
        homePageWallpaperPath: file.path,
