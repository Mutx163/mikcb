...
void main() {
  ...
  group('xiaoai API 集成测试', () {
    test('API 可访问且返回 200', () async {
      final uri = Uri.parse('https://publicapi.xiaoai.me/holiday/year?year=2026');
    ...
    test('数据包含 2026 年元旦', () async {
      ...
      // 查找 2026-01-01
      final newYear = data.where((item) =>
          (item as Map<String, dynamic>)['date'] == '2026-01-01').toList();
    ...
    test('数据包含 2026 年国庆节', () async {
      ...
      // 查找 2026-10-01
      final nationalDay = data.where((item) =>
          (item as Map<String, dynamic>)['date'] == '2026-10-01').toList();
    ...
    test('HolidayService 能正确解析 xiaoai 数据', () async {
      ...
      // 输出日志供人工检查
      print('xiaoai API 返回 ${data.entries.length} 条节假日数据');
      print('假期条目：${data.entries.where((e) => e.type == HolidayType.vacation).length}');
      print('调休条目：${data.entries.where((e) => e.type == HolidayType.adjustedWorkday).length}');
    }, skip: skipInCI);
  });
  ...
  group('Fallback 机制集成测试', () {
    test('两个 API 返回的假期日期一致', () async {
      // 获取 xiaoai 数据
      final xiaoaiUri = Uri.parse('https://publicapi.xiaoai.me/holiday/year?year=2026');
      final xiaoaiResp = await http.get(xiaoaiUri).timeout(const Duration(seconds: 15));
      ...
      // 获取 ailcc 数据
      final ailccUri = Uri.parse('https://holiday.ailcc.com/api/holiday/year/2026');
      final ailccResp = await http.get(ailccUri).timeout(const Duration(seconds: 15));
      ...
      final ailccHolidays = ailccHoliday.entries
          .where((entry) => (entry.value as Map<String, dynamic>)['holiday'] == true)
          .map((entry) => (entry.value as Map<String, dynamic>)['date'] as String)
          .toSet();

      // 比较（允许小差异，因为数据源可能略有不同）
      final commonHolidays = xiaoaiHolidays.intersection(ailccHolidays);
      print('xiaoai 假期数：${xiaoaiHolidays.length}');
      print('ailcc 假期数：${ailccHolidays.length}');
      print('共同假期数：${commonHolidays.length}');
    ...
    test('HolidayService fallback 真实场景', () async {
      ...
      // 验证日志显示使用了哪个 API
      final usedPrimary = service.logs.any((e) => e.message.contains('主 API 返回'));
      final usedFallback = service.logs.any((e) => e.message.contains('备用 API 返回'));
      final usedBuiltin = service.logs.any((e) => e.message.contains('内置资产'));

      print('使用主 API：$usedPrimary');
      print('使用备用 API：$usedFallback');
      print('使用内置资产：$usedBuiltin');
  ...
  group('数据质量验证', () {
    test('2026 年节假日数据完整性', () async {
      ...
      // 输出统计
      final vacationDays =
          data.entries.where((e) => e.type == HolidayType.vacation).length;
      final adjustedWorkdays =
          data.entries.where((e) => e.type == HolidayType.adjustedWorkday).length;
      print('2026 年假期天数：$vacationDays');
      print('2026 年调休天数：$adjustedWorkdays');
    }, skip: skipInCI);
    ...
    test('节假日条目有合理的 groupId', () async {
      ...
      print('假期分组数：${groups.length}');
      for (final group in groups.entries) {
        print('  ${group.key}: ${group.value.map((e) => e.name).toSet().join(", ")} (${group.value.length} 天)');
      }
    }, skip: skipInCI);
  });
}
