...
// 线条」）。参考实现 NexioSchedule 的边光始终是均匀的（`EdgeLightStyle.Uniform`，
// alpha 只按「离边多远」算、与方向无关），它的圆件（CircleShape 按钮）也靠折射而非
...
// —— 与 Nexio 课程表的 InteractiveHighlight 同款观感，强度写死在此文件里，
// 不开放调参（「一种观感」铁律；可调的只有「有没有按着」）。
//
