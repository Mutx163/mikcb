...
// ── 边光整圈均匀（与全局那份同口径） ──
//
// 全局表面（glass_surface_refraction.frag）在 2026-09-20 的第三、四轮里曾加过一条按
// 形状分档的规则（方正面板「只留转角」），**2026-09-21 已整体撤掉、两边都回到整圈均匀**。
// 这一份当年照抄过那条规则，2026-09-20 就撤掉了 —— 撤掉的实测依据留在这里，因为它是
// 全局那份后来也撤掉的直接理由：
//
