...
/// - [tintBottomScale]：衬底**下半段**保留的不透明度比例。0 = 只留
///   `[顶, 透明]` 两段（与接入调参前的观感逐像素一致）；>0 在两段之间插一个
///   `α × bottomScale` 的中间 stop，抬高下半段浓度。**末段一律收敛到 0**：
///   任何取值都只在带内加雾，不会在带底留下一条横向硬边（真机曾把那条边
///   读作「最浓状态下底部出现一条横向」）。
///
