...
class WidgetCourseAccent {
  ...
  /// 浅底文字带上限：实测对状态胶囊底（#E0EAFF / #EEF2F7）最低 4.78:1，优于
  /// 现状固定灰字 #64748B 的 3.94:1，也仍在 WCAG AA 的 4.5:1 之上。
  ///
  /// 这个值被两头夹着，0.13 是交点：
  /// - 往上抬能让「字跟色条同一个颜色」更成立，但没有余地——胶囊底 #E0EAFF 比
  ///   卡底更暗，是最严的那个底：0.13 时 4.78:1，0.14 只剩 4.52:1，0.16 直接
  ///   跌破 4.5；
  /// - 往下压（旧档 0.10）则压过了头：对白卡 7.00:1，远超达标线，代价是同一门
  ///   课的文字比色条最多暗 2.45 倍（104 色预设里 98 色如此，例 #5EEAD4 →
  ///   色条 #149883 / 文字 #0D6457），用户读到的就是「色块和字不是同一个颜色」。
  ///   收到该反馈后放宽到 0.13，最大差距收窄到 1.89 倍，胶囊底仍留 4.78:1。
  static const double lightTextMaxY = 0.13;
