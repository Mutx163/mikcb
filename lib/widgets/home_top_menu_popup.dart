        ...
        child: Stack(
          children: [
            ...
            MiuixGlassTransformPopup(
              ...
              // 右上）：让位朝右上角退，读起来是**向右退让**。旧实现
              // （`hyperos_list_popup.dart` 的 `Transform.scale` + 同一对角
              // `Alignment(1, -1)`）就是这个角，用户口径「原本是向右缩，那是
              // 最好的效果」（2026-09-20）。
              stackShrinkFromAnchor: true,
              // 让位**只缩内容，不缩卡片轮廓**（fork 补丁参数）。
              //
              // 为什么：二级面板是另一块独立面板，按一级里「添加」那一行锚定、
              // 自己不参与让位。若一级连卡片一起缩，一级卡的左边缘会往右收
              // 10px（200 宽 × 5%）、下边缘往上收 14px，而二级卡原地不动 ——
              // 两块卡的左边就差出一条 10px 的缝，真机反馈读成「玻璃缩小了，
              // 但下面那张实体卡没缩」（2026-09-20 实测：一级卡左边缘
              // 200.5 → 210.5，二级卡仍是 200.5）。只缩内容时一级卡轮廓原地
              // 不动、与二级卡左边对齐，「退让」的层次感由压暗交代。
              //
              // 代价：内容缩进去后卡片左 / 下边缘比内容宽出一截（留出空隙），
              // 且二级面板顶部那个「父行复印件」仍与一级那一行错开几像素
              // （离支点越远挪得越多，200 宽面板上最多 9px）—— 这是**让位设计
              // 本身**（旧实现测试注释原话：「面板向锚点角回放缩放 0.95、父行
              // 视觉后退属于让位设计」），不是缺陷，别再为它去动支点：试过把
              // 支点钉在那一行上（fork 的 `stackPivotBounds`），退让方向会变成
              // 向左缩、且用户报的"两个一级卡片"与支点无关（那是共享捕获组的
              // 问题，见上面垫底那段）。
              stackScalesPanel: false,
