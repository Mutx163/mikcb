        ...
        child: Stack(
          children: [
            ...
            MiuixGlassTransformPopup(
              ...
              // 这个让位的本体**，内容跟着缩是理所当然的。
              //
              // ⚠️ **二级面板不参与让位**（2026-09-21 用户口径，已发布预发布版
              // v2.1.2.3 就是这个观感）：一级缩 5% 后左边缘比二级多收 5% 板宽
              // （200 宽上是 10px），接缝就在两块卡的左边缘。用户明确选择接受这条
              // 缝 —— 代价是固定的，别再拿「二级也缩」「只缩内容」去消它：前者会
              // 让二级的字一起小 5%、被点那一行跟着挪（用户否），后者已被否过
              // （见 `rejected/bug-fix/2026-09-20-home-menu-stack-scales-content-only.md`）。
            ...
            if (secondaryBounds != null)
              MiuixGlassSecondaryPopup(
                ...
                // 用户已在同一轮明确选择接受（见一级面板 `stackShrinkFromAnchor`
                // 那段注释）。
                //
