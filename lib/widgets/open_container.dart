...
//
// mikcb vendored copy of package:animations 2.2.0 lib/src/open_container.dart
// (Apache-2.0, unchanged except the perf patch marked "mikcb patch (perf)"
// below). Reason: the stock buildPage invokes closedBuilder/openBuilder
// INSIDE the per-frame AnimatedBuilder, so a 420ms container transform
// rebuilds the entire open page (AddCourseScreen is a full form) and the
// closed agenda card on every animation tick — probe-verified 20 calls per
// 300ms transition, which blew the UI-thread frame budget and made the
// day-view course-detail expand read as a slideshow.
//
// 上游若发布修复(转场期间不再逐帧调用 builders)可删除本文件并还原
// package:animations 导入。
//
// 以下两条 lint 针对的是上游代码自身的字面量/判空风格,与补丁无关,
// 保持 vendored 文件与上游最小 diff,不做机械改写:
// ignore_for_file: prefer_int_literals, prefer_null_aware_method_calls
...
class _OpenContainerRoute<T> extends ModalRoute<T> {
  ...
  // mikcb patch (perf): content subtrees built ONCE and reused as identical
  // widget instances across animation frames. The stock code re-invoked
  // closedBuilder/openBuilder every tick (probe: 20 calls / 300ms), which
  // rebuilt the whole open page + closed card per frame. With identical
  // instances the element diff short-circuits and only the lightweight morph
  // shell (Material / FittedBox / FadeTransition / tweens) rebuilds per tick;
  // FadeTransition still updates layer alpha via its own animation listenable.
  Widget? _cachedClosedContent;
  Widget? _cachedOpenContent;

  Widget get _closedContentOnce =>
      _cachedClosedContent ??= Builder(
        key: closedBuilderKey,
        builder: (BuildContext context) {
          // Use dummy "open container" callback since we are in the process
          // of opening.
          return closedBuilder(context, () {});
        },
      );

  Widget get _openContentOnce =>
      _cachedOpenContent ??= Builder(
        key: _openBuilderKey,
        ...
        builder: (BuildContext context, Widget? child) {
          if (animation.isCompleted) {
            final Widget material = Material(
              ...
              shape: openShape,
              // mikcb patch (perf): cached instance — the GlobalKey keeps the
              // open page's state alive across the morph→full slot switch.
              child: _openContentOnce,
            );
                  ...
                  child: SizedBox(
                    ...
                    child: (hideableKey.currentState?.isInTree ?? false)
                        ...
                        : FadeTransition(
                            ...
                            // mikcb patch (perf): cached instance.
                            child: _closedContentOnce,
                          ),
                  ),
                ),
